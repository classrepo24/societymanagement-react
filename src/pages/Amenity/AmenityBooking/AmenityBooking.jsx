...
export const AmenityBooking = () => {
  ...
  // Context
  const { bookings, setBookings, amenities, setAmenities } = useAmenity();
  ...
  const currentAmenity = amenities.find((item) => item.id === amenity.id);
  useEffect(() => {
    if (currentAmenity && currentAmenity.availability !== "Available") {
      navigate("/amenities");
    }
  }, [currentAmenity]);
  ...
  const isSpecialTiming =
    currentAmenity?.availabilitySettings?.specialTimings?.some((item) => {
      if (item.dateType === "Single Date") {
        return item.fromDate === selectedDateString;
      }

      return (
        selectedDateString >= item.fromDate && selectedDateString <= item.toDate
      );
    });
  ...
  const bookingSlots = useMemo(() => {
    ...
    const todayAvailability = currentAmenity?.weeklyAvailability?.find(
      (day) => day.day === dayName,
    );

    const todaySlots = currentAmenity?.timeSlots;

    if (!todayAvailability) return [];

    if (todayAvailability.status === "Closed") {
      return [];
    ...
    return todaySlots.map((slot, index) => {
      ...
      const operatingStart = currentAmenity?.operatingHours?.start;

      const operatingEnd = currentAmenity?.operatingHours?.end;
      ...
      const isWithinOperatingHours =
        !operatingStart ||
        !operatingEnd ||
        (slotStart >= operatingStart && slotStart < operatingEnd);
  ...
  const securityDeposit = currentAmenity?.additionalSettings
    ?.requireSecurityDeposit
    ? Number(currentAmenity.additionalSettings.securityDepositAmount || 0)
    : 0;
  console.log("ADDITIONAL SETTINGS", currentAmenity?.additionalSettings);

  console.log("SECURITY DEPOSIT", securityDeposit);

  const totalAmount = useMemo(() => {
    return selectedSlot ? pricePerHour + cleaningCharges + securityDeposit : 0;
  }, [selectedSlot, pricePerHour, securityDeposit]);
  // Breadcrumb
  ...
  const handleConfirmBooking = () => {
    ...
    maxBookingDate.setDate(
      today.getDate() + Number(currentAmenity?.advanceBookingDays || 0),
    );

    if (selectedDate > maxBookingDate) {
      newErrors.date = `Booking allowed only ${currentAmenity?.advanceBookingDays} days in advance`;
    }

    if (
      todayBookingsCount >=
      Number(currentAmenity?.maxBookingsPerDay || Infinity)
    ) {
      newErrors.limit = `Maximum ${currentAmenity.maxBookingsPerDay} bookings allowed per day`;
    }
    ...
    if (
      currentAmenity?.additionalSettings?.requirePurpose &&
      !formData.purpose
    ) {
    ...
    if (
      currentAmenity?.additionalSettings?.requirePeopleCount &&
    ...
    if (!formData.agreed) {
      newErrors.agreed = "Please accept the rules & guidelines";
    }
    ...
    localStorage.setItem("amenityBookings", JSON.stringify(updatedBookings));
    setAmenities((prev) =>
      prev.map((item) => {
          ...
          <div className="flex flex-1 flex-col justify-between">
            <div>
              ...
              <div className="mt-6 grid grid-cols-6 gap-5">
                <div>
                  <p className="text-[13px] text-[#64748B]">Category</p>
                ...
                <div>
                  <p className="text-[13px] text-[#64748B]">Capacity</p>
                ...
                <div>
                  <p className="text-[13px] text-[#64748B]">Booking Price</p>
                ...
                <div>
                  <p className="text-[13px] text-[#64748B]">Advance Notice</p>

                  <h4 className="mt-1 text-[18px] font-semibold text-[#0F172A]">
                    {currentAmenity?.advanceBookingDays || 0}
                  </h4>
                </div>

                <div>
                  <p className="text-[13px] text-[#64748B]">Booking Type</p>
        ...
        <div className="col-span-6">
          {bookingSlots.length === 0 ? (
            <div className="p-8 rounded-xl border border-red-200 bg-red-50 text-center">
              ...
              <p className="text-gray-500 mt-2">Please choose another date.</p>
            </div>
          ) : (
            <TimeSlotGrid
